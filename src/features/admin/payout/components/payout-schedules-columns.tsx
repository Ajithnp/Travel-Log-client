...
import { Clock, Eye, IndianRupee, User } from "lucide-react";
            ...
            render: (schedule) => (
                <div className="flex flex-col">
                    <span className="font-medium text-orange-500 flex items-center gap-2">
                        <IndianRupee className="w-3 h-3 text-emerald-600" />
                        {schedule.netAmount + schedule.totalRefundedAmount}
                    </span>
                </div>
            ),
        },
            ...
            render: (schedule) => {
                ...
                return (
                    <div className="flex flex-col">
                        ...
                        {
                            schedule.alreadyFailed ? (
                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium w-fit text-red-700 bg-red-100/80">
                                    <span className="w-1.5 h-1 rounded-full bg-red-500" />
                                    Failed
                                </span>
                            ) : (
...

        {
            ...
            render: (schedule) => {
                const actions: TableAction[] = [
                    {
                        label: schedule.alreadyFailed ? 'Go to history to retry' : 'Pay',
                        icon:
                        ...
                        disabled: !schedule.readyToPayout || schedule.alreadyFailed,
                        variant: schedule.alreadyFailed ? 'danger' : 'success',
