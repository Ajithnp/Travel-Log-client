...
function StatCard({ label, value, subtitle, color, delay = 0 }: { label: string; value: number | string; subtitle: string; color?: string; delay?: number }) {
  return (
    <motion.div 
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5, delay, ease: "easeOut" }}
      className="bg-card rounded-xl border shadow-premium flex flex-col gap-2 p-6"
    >
